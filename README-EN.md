...
| [knowledge-mcp](https://github.com/chirag127/knowledge-mcp) | Public MCP server exposing an OKF (Open Knowledge Format) bundle over `search`, `read`, `list`, `related` tools. Runs on Cloudflare Workers, no auth, no rate limit, embeds 800+ concept files as JSON at build. Any MCP client (Claude Code, Cursor, etc.) can point at the endpoint and query the knowledge graph. | <https://knowledge-mcp.oriz.in> | Maintaining |
